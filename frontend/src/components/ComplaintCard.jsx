    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-200 dark:border-gray-800 flex flex-col h-[550px]">
      ...
      <div className="relative w-full h-48 overflow-hidden">
        {c.imageUrl ? (
          ...
          <div className="w-full h-full bg-gray-200 dark:bg-gray-800 flex items-center justify-center text-gray-500 text-sm">
            No Image
          </div>
        )}
        ...
        {c.status && (
          <span
            className={`absolute top-3 right-3 px-3 py-1 rounded-full text-xs font-semibold border backdrop-blur-sm ${getStatusStyles(
              c.status
            )}`}
          >
            {c.status}
